defmodule FranklinWeb.Admin.Articles.IndexLive do
  ...
  def render(assigns) do
    ...
    <p class="my-4">
      <.link navigate={~p"/admin/articles/editor/new"}>New Article</.link>
    </p>

    <.admin_simple_table rows={@articles}>
      <:column :let={article} label="Title">
        <.link navigate={~p"/admin/articles/#{article.id}"}><%= article.title %></.link>
      </:column>
      <:column :let={article} label="Published At">
        <%= article.published_at %>
      </:column>
      <:column :let={article} label="Edit">
        <.link navigate={~p"/admin/articles/editor/#{article.id}"}>Edit</.link>
      </:column>
    </.admin_simple_table>
    """
  end
